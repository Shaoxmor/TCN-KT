...
tf.flags.DEFINE_integer("epochs", 32, "Number of epochs to train for.")
tf.flags.DEFINE_string("train_data_path", 'data/assist2009_updated/assist2009_updated_train1.csv', "Path to the training dataset")
tf.flags.DEFINE_string("valid_data_path", 'data/assist2009_updated/assist2009_updated_valid1.csv', "Path to the testing dataset")
tf.flags.DEFINE_string("information_data_path", 'data/assist2009_updated/clustered_skill_name.txt', "Path to the information dataset")
...
def train():
    ...
    test_students, test_max_num_problems, test_max_skill_num = dh.read_data_from_csv_file(FLAGS.valid_data_path)
        ...
        with sess.as_default():
            ...
            with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
                learning_rate = tf.train.exponential_decay(learning_rate=FLAGS.learning_rate,
                                                           ...
                                                           decay_steps=(len(train_students)//FLAGS.batch_size +1) * FLAGS.decay_steps,
                                                           decay_rate=FLAGS.decay_rate, staircase=True)
                optimizer = tf.train.AdamOptimizer(learning_rate)
                train_op = optimizer.minimize(tcn.loss, global_step=tcn.global_step, name="train_op")
            ...
            def train_step(x, next_id, target_id, target_correctness, same,differ):
                feed_dict = {
                    tcn.input_data: x,
                    tcn.next_id: next_id,
                ...
                _, step, summaries, pred, loss = sess.run(
                    [train_op, tcn.global_step, train_summary_op, tcn.pred, tcn.loss], feed_dict)
                train_summary_writer.add_summary(summaries, step)
                return pred, loss

            def validation_step(x,  next_id, target_id, target_correctness, same,differ):
                feed_dict = {
                    tcn.input_data: x,
                    tcn.next_id: next_id,
                    tcn.target_id: target_id,
                ...
                step, summaries, pred, loss = sess.run(
                    [tcn.global_step, validation_summary_op, tcn.pred, tcn.loss], feed_dict)
            ...
            for iii in range(FLAGS.epochs):
                ...
                while(index+FLAGS.batch_size < data_size):
                    ...
                    pred, loss = train_step(x, next_id, target_id, target_correctness, same,differ)
                    for p in pred:
                    ...
                    while(index+FLAGS.batch_size < data_size):
                        ...
                        for i in range(FLAGS.batch_size):
                            ...
                            for j in range(len(problem_ids)-1):
                                ...
                                actual_labels.append(int(correctness[j+1]))
                        index += FLAGS.batch_size
                        pred  = validation_step(x, next_id, target_id, target_correctness, same,differ)
                        for p in pred:
                                pred_labels.append(p)
                    ll = data_size - index
                    x = np.zeros((ll, max_num_steps, ))
